...
public:
    ...
    void clearGroups(){
        while (groupBoid.size()>0) {
            delete groupBoid[0]; // il faut viré les Boid du group !!! ahahahahh
    ...
    void addGoup(){
        ...
        for (int i=0; i<groupBoid.size()-1; i++) { /// modif du - 1 pour ne pas rajouté une regle sur le dernier gourp
            GroupBoid2d * g=groupBoid.at(i);
            g->addRegle();
        }
    }
    
    void addGroups(int _numGroups){
...
///////////////////////////////// Attractions ///////////////////
    ...
    // creation d'une surcharge
    Flock2d * addAttrationLine(ofPoint _a, ofPoint _b, float _force, float _sensorDist, float _lengthZone, int _group){
        AttractionLine2d *al = new AttractionLine2d( _a, _b, _force, _sensorDist, _lengthZone, _group);
